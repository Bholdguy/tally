...
| Item | Status |
|---|---|
| Real-speech validation pass (3+ human speakers, 10+ recordings each, criteria fixed in advance) | **Run 2026-09-23. OVERALL: FAIL** (`docs/real-speech-validation-results.md`; 3 speakers, 36 recordings). The zero-tolerance criterion held — 0 orders ever confirmed wrong. But the false-hold rate (24.6% vs 15%), clean-room evidence accuracy (66.7% vs 95%), and the evidence-wait bound (once, by 18 ms) all missed their fixed thresholds. **Say this plainly if asked: real speech is measurably harder than the synthetic/captured phrasing used elsewhere, and Step 6 is not demo-ready by its own gate.** |
