...
| 8b | **Fail closed: the evidence stream dies** | VALIDATE (fail closed) | **Play scenario: dropout** | Red **⛔ EVIDENCE STREAM DOWN → calls are HELD (fail closed)** within a fraction of a second, not after the 4.0 s timeout; `HELD UNVALIDATABLE`; order empty. Say: *"No evidence, no commit. It doesn't hang and it doesn't guess."* |
...
- **Latency:** measured (spike-g5/A): end of customer speech → first agent audio ≈ 2.7 s p50 / 8.4 s p95, dominated by the agent's own turn detection and model time, not gating. When the gate has to wait for evidence the wait is on screen (beat 5b), bounded at 4.0 s (briefly raised to 4.5 s on 2026-09-23, reverted the same day — see D-26). Do not promise "sub-second".
...
| Item | Status |
|---|---|
| Real-speech validation pass (3+ human speakers, 10+ recordings each, criteria fixed in advance) | **Run 2026-09-23, over five runs. FINAL state — OVERALL: FAIL.** (`docs/real-speech-validation-results.md`, full history in `DECISIONS.md` D-26; 3 speakers, 36 recordings). **P1 (zero confirmed-wrong orders): solid PASS every run, no exceptions.** First run: false-hold rate 24.6% and clean-room evidence accuracy 66.7% both missed target. Fixed a missing language hint on the independent STT stream and two extractor quantity-parsing bugs, re-ran: false-hold rate passed (13.8%); evidence accuracy improved to 80.0% but still missed the 95% target. A third run tried raising the wait ceiling 4 s→4.5 s; that reasoning was wrong (a higher ceiling can only push *observed* waits higher, never satisfy a fixed lower bound) and was reverted the same day. Two further runs at the reverted 4 s ceiling, no code changes, resolved an open question: **the false-hold rate (P3/P3b) is genuinely UNSTABLE run to run against the real managed agent** — four runs at the identical ceiling landed at 24.6%, 13.8%, 14.6%, 24.1%, spanning both sides of the 15% budget, with the raw call count itself churning on byte-identical audio. Say this as **"borderline, agent-dependent," not a fixed pass or fail** — confirmed genuine non-determinism in the live, non-seedable managed agent, not a Tally defect. **Evidence accuracy (P4) remains a real, understood gap**, 76.7-80.0% against a 95% target — accented/disfluent real English exceeding what language-hint steering alone resolves; no further ASR tuning attempted given the deadline. **The evidence-wait bound (P5) fails by a small, consistent, structural margin** — 12-27 ms over the 4 s ceiling, every single run, regardless of which ceiling was active — real managed-agent latency, not a fixable Tally timing bug. **Say this plainly if asked: P1/P6/P7 are solid, P4 is a real and understood gap, P5 is a small structural latency margin, and P3/P3b are unstable rather than reliably passing or failing — and Step 6 is not demo-ready by its own gate.** |
