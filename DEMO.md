...
| Item | Status |
|---|---|
| Real-speech validation pass (3+ human speakers, 10+ recordings each, criteria fixed in advance) | **Run 2026-09-23, then re-run after two root-cause fixes. OVERALL: still FAIL** (`docs/real-speech-validation-results.md`; 3 speakers, 36 recordings). The zero-tolerance criterion held throughout — 0 orders ever confirmed wrong. First run: false-hold rate 24.6% and clean-room evidence accuracy 66.7% both missed target. Fixed a missing language hint on the independent STT stream and two extractor quantity-parsing bugs, re-ran the same 36 recordings: **false-hold rate now passes (13.8%)**; evidence accuracy improved to 80.0% but still misses the 95% target; the evidence-wait bound still marginally misses (unrelated to either fix). **Say this plainly if asked: real speech is measurably harder than the synthetic/captured phrasing used elsewhere — better than it was, still not there — and Step 6 is not demo-ready by its own gate.** |
