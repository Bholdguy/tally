...
| 8b | **Fail closed: the evidence stream dies** | VALIDATE (fail closed) | **Play scenario: dropout** | Red **⛔ EVIDENCE STREAM DOWN → calls are HELD (fail closed)** within a fraction of a second, not after the 4.5 s timeout; `HELD UNVALIDATABLE`; order empty. Say: *"No evidence, no commit. It doesn't hang and it doesn't guess."* |
...
- **Latency:** measured (spike-g5/A): end of customer speech → first agent audio ≈ 2.7 s p50 / 8.4 s p95, dominated by the agent's own turn detection and model time, not gating. When the gate has to wait for evidence the wait is on screen (beat 5b), bounded at 4.5 s (raised from 4 s 2026-09-23). Do not promise "sub-second".
...
- "Waiting on independent evidence" is a first-class UI state: chip, live countdown against `EVIDENCE_WAIT_MAX_MS` (4.5 s), both transcripts side by side, for the whole wait.
...
| Item | Status |
|---|---|
| Real-speech validation pass (3+ human speakers, 10+ recordings each, criteria fixed in advance) | **Run 2026-09-23, then re-run twice more (two root-cause fixes, then a targeted config fix). OVERALL: still FAIL** (`docs/real-speech-validation-results.md`; 3 speakers, 36 recordings). The zero-tolerance criterion held throughout, all three runs — 0 orders ever confirmed wrong. First run: false-hold rate 24.6% and clean-room evidence accuracy 66.7% both missed target. Fixed a missing language hint on the independent STT stream and two extractor quantity-parsing bugs, re-ran the same 36 recordings: false-hold rate passed (13.8%); evidence accuracy improved to 80.0% but still missed the 95% target; the evidence-wait bound marginally missed (18-22 ms over a 4 s ceiling). Raised the system's own evidence-wait ceiling 4 s→4.5 s (small, consistent real-agent latency variance, not a design flaw) and re-ran once more: **the wait-bound criterion still fails** — its fixed 4 s threshold is unchanged by design, so it cannot pass once the system is deliberately allowed to wait longer (honest max wait now 4508 ms); **the false-hold rate regressed back to FAIL (23.2%)** on the identical recordings with no extractor/STT change, most likely ordinary run-to-run managed-agent variance (unconfirmed). Evidence accuracy held at 80.0% — no further ASR tuning attempted given the deadline; root cause is accented/disfluent real English exceeding what language-hint steering alone resolves. **Say this plainly if asked: real speech is measurably harder than the synthetic/captured phrasing used elsewhere, and Step 6 is not demo-ready by its own gate — evidence accuracy is the persistent gap, measured at 80% against a 95% target.** |
