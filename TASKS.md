...
- [x] 🚧 **Validation condition 2 — live-mic session exercising the ACTUAL repair loop and barge-in over the agent:** run 2026-09-23, real human, real microphone, real managed agent (`docs/live-repair-validation.md` Part 2, case `bbc2f4a7`). (a) scoped question PASS, (b) re-issued/re-validated commit PASS. **(c) two-failed-asks hand-off and (d) barge-in were NOT exercised by a human this run** (both are proven only by the automated/induced form, Part 1, n=1-3, one synthetic voice). Condition 2 is satisfied for (a)/(b) against real human speech; (c)/(d) against real human speech remain open.
