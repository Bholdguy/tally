...
- [ ] 🚧 **Audio tier not yet run against the REAL managed agent.** It is proven with mock Voice Agent/STT servers only (wiring, pacing, k-accounting, judging). A real run needs a real resolved case, i.e. a real hold + resolution — **that precondition is now met** (case `bbc2f4a7`, live human mic session 2026-09-23, `docs/live-repair-validation.md` Part 2), but the audio-tier replay itself has not yet been run against the real managed agent for that case. Until it is, no claim is made about real-agent repeatability
