...
- [x] 🚧 **Real-speech validation pass: RUN 2026-09-23, then re-run after two root-cause fixes. OVERALL: still FAIL** against its own fixed criteria (`docs/real-speech-validation-results.md`; 3 speakers, 36 recordings). P1/P6/P7 PASS. **P3/P3b (false-hold rate) now PASS** after fixing two real extractor bugs (a quantity leaking onto a conjoined item; an orphan correction number stolen by a later same-utterance mention). **P4 (evidence accuracy) improved 66.7%→80.0%** after adding `language_codes: ["en"]` to the independent STT stream (it had none, so it ran fully unsteered multilingual) but still FAILS the 95% target. P5 unchanged (unrelated to either fix; run-to-run agent variance). Not blocking Step 4's completion. **Step 6 is still NOT demo-ready per this gate** (D-26) — see the update there for the full before/after
...
- [x] 🚧 **Real-speech validation pass: RUN 2026-09-23, OVERALL still FAIL after root-cause fixes and a re-run.** 3 speakers, 36 recordings, `docs/real-speech-validation-results.md`. P1 (no wrong order ever confirmed) held throughout. False-hold rate (P3/P3b) now PASSES after fixing two extractor bugs; evidence accuracy (P4) improved (66.7%→80.0%) after fixing missing language steering on the independent STT stream but still fails its 95% target; the evidence-wait bound (P5) is unaffected by either fix and still fails marginally. **Step 6 is not demo-ready. Stated plainly in README, DEMO.md and the Lab tab, not softened**
