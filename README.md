...
3. **The live repair loop:** validated once automatically (real agent + real STT, a synthetic customer voice, holds induced by fault injection; small sample), **and once by a human on a real microphone** (2026-09-23, `docs/live-repair-validation.md` Part 2): the correction was caught, the call held correctly, the repair question fired and resolved correctly. **Barge-in and the two-failed-asks hand-off have still never been exercised by a real human** — only by the automated/induced form.
