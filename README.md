...
1. **Validated on real speech, 2026-09-23 — and it failed its own fixed criteria.** 3 speakers, 36 recordings (`docs/real-speech-validation-results.md`). The zero-tolerance criterion held (0 confirmed wrong orders, ever). But real disfluent speech (false starts, filler words) produced a false-hold rate of 24.6% against a 15% budget, clean-room evidence-extraction accuracy was 66.7% against a 95% target, and one call's evidence wait exceeded the 4 s bound by 18 ms. Real speech is measurably harder for this system than the synthetic and captured phrasing every earlier claim in this document was based on. **Accuracy claims elsewhere in this repo predate this result and should be read as synthetic/captured-phrasing claims, not real-speech claims**, until they're revisited.
